...
from project.utils import make_hist, reverse_dict
...
class AgentBuildings(ThermalBuildings):
    ...
    def frame_to_flow(self, replaced_by):
        ...
        for component in ['Wall', 'Floor', 'Roof', 'Windows']:
            ...
            replaced_by.loc[replaced_by['{} after'.format(component)], component] = self._performance_insulation[
                component]
        ...
        for policy in policies_heater:
            ...
            if policy.policy == 'subsidy_target':
                ...
                if isinstance(value, DataFrame):
                    sub = (value * cost_heater).loc[:, cost_heater.index]
        ...
        def calibration_constant_heater(_utility, _ms_heater):
            ...
            for i in range(50):
                ...
                if 'Electricity-Heat pump air' in constant.index:
                    constant.loc[constant['Electricity-Heat pump water'].isna() & constant[
                        'Electricity-Heat pump air'].notna(), 'Electricity-Heat pump air'] = 0
            ...
            details = concat((constant.stack(), market_share_ini.stack(), market_share_agg.stack(), _ms_heater.stack(),
                              agg.stack() / 10 ** 3, wtp.stack()),
                             axis=1, keys=['constant', 'calcul ini', 'calcul', 'observed', 'thousand', 'wtp']).round(
                decimals=3)
            if self.path is not None:
    ...
    def store_information_heater(self, cost_heater, subsidies_total, subsidies_details, replacement, vta_heater,
        ...
        for key, sub in self._heater_store['subsidies_details'].items():
            mask = sub.copy()
            mask[mask > 0] = 1
            self._heater_store['subsidies_count'].update(
                {key: (replacement.fillna(0) * mask).sum(axis=1).groupby('Housing type').sum()})
    ...
    def heater_replacement(self, stock, prices, cost_heater, lifetime_heater, policies_heater, ms_heater=None,
                           ...
                           prices_before=None, supply=None, store_information=True):
        ...
        idx = (subsidies_total.index.get_level_values('Performance').isin(['F', 'G'])) & (
            ~subsidies_total.index.get_level_values('Heating system').isin(self._resources_data['index']['Heat pumps']))
    ...
    def apply_subsidies_insulation(self, index, policies_insulation, cost_insulation, surface, certificate,
                                   ...
                                   consumption_saved, calculate_condition=True):
        ...
        if calculate_condition:
            self._list_condition_subsidies = [i.target for i in policies_insulation if
                                              i.target is not None and i.policy != 'subsidies_cap' and isinstance(i.target,
                                                                                                                  str)]
    ...
    def endogenous_renovation(self, stock, prices, subsidies_total, cost_insulation,
        ...
        def to_market_share(_bill_saved, _subsidies_total, _cost_total, _cost_financing=None):
        ...
        def to_expected_utility(_cost_total, _bill_saved, _subsidies_total, _market_share, _utility_intensive=None,
                                _cost_financing=None, option=None):
            ...
            # extensive margin
            if option == 'log_sum':
                return log(exp(_utility_intensive).sum(axis=1))

            else:
                _bill_saved_insulation, _subsidies_insulation, _investment_insulation, _financing_insulation = None, None, None, None
                if option == 'market_share':
                    ...
                    if _cost_financing is not None:
                        _financing_insulation = (_cost_financing.reindex(_market_share.index) * _market_share).sum(
                            axis=1)
                ...
                utility_bill_saving = reindex_mi(self.preferences_insulation['bill_saved'],
                                                 _bill_saved_insulation.index) * _bill_saved_insulation / 1000
                ...
                if _financing_insulation is not None:
                    _financing_insulation.drop(idx, inplace=True)
                    pref_financing = reindex_mi(self.preferences_insulation['cost'],
                                                _financing_insulation.index).rename(None)
                    utility_financing = (pref_financing * _financing_insulation) / 1000
                    expected_utility += utility_financing
        ...
        def calibration_intensive(util, _stock, market_share_ini, _renovation_rate, iteration=1000):
            ...
            details = concat(
                (const, ms_ini, ms_agg, market_share_ini, (market_share_ini * nb_renovation) / 10 ** 3, wtp), axis=1,
        ...
        def calibration_extensive(_utility, _stock, _calib_renovation, option='market_share'):
            ...
            def solve_deviation(x, utility_ini, stock_ini, retrofit_rate_target, std_deviation):
                ...
                std_deviation_calc = (((retrofit_rate_calc - retrofit_mean) ** 2 * stock_ini).sum() / (
                            stock_ini.sum() - 1)) ** (1 / 2)
                rslt = append(rslt, std_deviation - std_deviation_calc)

                return rslt

            def solve_deviation_logsum(x, utility_ini, stock_ini, retrofit_rate_target, std_deviation):
                scale = x[-1]
                cst = x[:-1]
                ...
                if calibration_scale['name'] == 'freeriders':
                    ...
                    if option != 'log_sum':
                        root, info_dict, ier, mess = fsolve(solve_deviation, x, args=(
                            _utility, stock_retrofit, retrofit_rate_ini, calibration_scale['deviation']),
                                                            full_output=True)
                    else:
                        root, info_dict, ier, mess = fsolve(solve_deviation_logsum, x, args=(
                            _utility, stock_retrofit, retrofit_rate_ini, calibration_scale['deviation']),
                                                            full_output=True)
                        _utility = log(exp(_utility * root[-1]).sum(axis=1))
        ...
        def calibration_coupled(_stock, _cost_total, _bill_saved, _subsidies_total, _calib_renovation,
                                _calib_intensive, _cost_financing=None, _supply=None):
            self.logger.info('Calibration intensive and renovation rate')

            if _supply is not None:
                _cost_total *= _supply['markup_insulation']
            ...
            for k in range(15):
                ...
                if self._expected_utility != 'log_sum' or utility_intensive is None:
                    ...
                    constant, scale = calibration_extensive(utility_intensive, _stock, _calib_renovation,
                                                            option='log_sum')
                ...
                if self._expected_utility != 'log_sum' or utility_intensive is None:
                    expected_utility = to_expected_utility(_cost_total, _bill_saved, _subsidies_total, _market_share,
                                                           _cost_financing=_cost_financing, option='market_share')
                else:
                    expected_utility = log(exp(utility_intensive * scale).sum(axis=1))
        ...
        def calibration_coupled_test(_stock, _cost_total, _bill_saved, _subsidies_total, _calib_renovation,
            ...
            def calibration(x, _stock, _cost_total, _bill_saved, _subsidies_total, ms_insulation_ini,
                            renovation_rate_ini, std_deviation):
                ...
                constant_renovation = pd.Series(
                    x[len(ms_insulation_ini):len(renovation_rate_ini) + len(ms_insulation_ini)],
                ...
                utility_intensive = (
                                                utility_bill_saving + utility_investment + utility_subsidies) * scale + constant_insulation_intensive
            ...
            root, info_dict, ier, mess = fsolve(calibration, x, args=(
                _stock, _cost_total, _bill_saved, _subsidies_total, ms_insulation_ini, renovation_rate_ini,
                std_deviation), full_output=True)
        ...
        if self.constant_insulation_intensive is None and self.rational_behavior is None:
            ...
            calibration_coupled(stock, cost_insulation.copy(), bill_saved.copy(), subsidies_total.copy(),
                                calib_renovation, calib_intensive,
                                _cost_financing=cost_financing, _supply=supply)
    ...
    def insulation_replacement(self, stock, prices, cost_insulation_raw, policies_insulation=None, financing_cost=None,
        ...
        cost_insulation, vta_insulation, tax, subsidies_details, subsidies_total, condition = self.apply_subsidies_insulation(
            ...
            certificate_before_heater, energy_saved_3uses, consumption_saved, calculate_condition=calculate_condition)
        ...
        if self._endogenous:
            ...
            renovation_rate, market_share = self.endogenous_renovation(stock, prices, subsidies_total,
                                                                       ...
                                                                       cost_financing=cost_financing, discount=discount,
    ...
    def parse_output_run(self, prices, inputs, climate=None, step=1, taxes=None, detailed_output=True,
        ...
        def marginal_abatement_cost(_consumption_saved, _cost_insulation, _stock, _prices, lifetime=lifetime,
                                    discount_rate=discount_rate, option=None, plot=False, carbon_saved=None,
                                    health_cost=None, indicator='abatement_cost'):
            ...
            _index = _consumption_saved.index
            cost_annualized = reindex_mi(cost_annualized, _index)
            _cost_insulation = reindex_mi(_cost_insulation, _index)
            discount_factor = reindex_mi(discount_factor, _index)

            _, _, _certificate = self.prepare_consumption(cost_annualized.columns, index=cost_annualized.index,
                                                          level_heater='Heating system', full_output=True)
            _output_statistics = {}
            health_cost_saved = None
            if health_cost is not None:
                s = self.add_certificate(_stock.loc[_index])
                health_cost_before = s * reindex_mi(health_cost, s.index).fillna(0)

                c = reindex_mi(_certificate, _index)
                health_cost_after = {}
                for i, j in c.items():
                    df = concat((j, _stock.loc[_index]), keys=['Performance', 'Stock'], axis=1).dropna().set_index('Performance', append=True).squeeze()
                ...
                health_cost_saved = (health_cost_saved.T / _stock.loc[_index]).T

            npv, roi = None, None
            if option is None:
                insulation = MultiIndex.from_frame(DataFrame(INSULATION))
                _consumption_saved_efficiency = _consumption_saved.loc[:, insulation]
                cost_annualized = cost_annualized.loc[:, insulation]

                if carbon_saved is not None:
                    carbon_saved = reindex_mi(carbon_saved, _index)
                ...
                cost_efficiency = cost_annualized / _consumption_saved_efficiency

                _consumption_saved_efficiency = (_stock * _consumption_saved_efficiency.T).T
                _consumption_saved_efficiency = _consumption_saved_efficiency.stack(_consumption_saved_efficiency.columns.names).squeeze()
                ...
                dict_df = {'consumption_saved': _consumption_saved}
                cash_flow = _bill_saved
                ...
                dict_df.update({'cash_flow': cash_flow, 'cost_insulation': _cost_insulation})

                _condition = _certificate.isin(['A', 'B']).replace(False, float('nan'))
                _index = _certificate[_condition.isnull().all(1)].index
                _condition.dropna(how='all', inplace=True)
                _condition = concat((_condition, _certificate.loc[_index, :].isin(['C'])), axis=0)
                ...
                _consumption_saved_efficiency = _stock * rslt_efficiency['consumption_saved']
                _output_statistics.update({'efficiency': (cost_efficiency * _stock).sum() / _stock.sum()})

                if isinstance(discount_factor, Series):
                    npv = - _cost_insulation + (discount_factor * cash_flow.T).T
                else:
                    npv = - _cost_insulation + discount_factor * cash_flow
                ...
                _consumption_saved_npv = _stock * rslt_npv['consumption_saved']
                _output_statistics.update({'npv': (npv * _stock).sum() / _stock.sum()})

                roi = _cost_insulation / cash_flow
                ...
                _consumption_saved_roi = _stock * rslt_roi['consumption_saved']
                _output_statistics.update({'roi': (roi * _stock).sum() / _stock.sum()})

                """cash_flow = rslt_npv['cash_flow']
                _cost_insulation = rslt_npv['cost_insulation']
                _cost_insulation = _cost_insulation[cash_flow > 0]
                cash_flow = cash_flow[cash_flow > 0]
                (cash_flow > _cost_insulation).any()

                x0 = Series([0.05] * cash_flow.shape[0], index=cash_flow.index)
                irr = fsolve(lambda x: ((1 - (1 + x) ** -lifetime) / x) * cash_flow - _cost_insulation, x0=x0)"""


            # multiply cost_saved by stock cost_efficiency do not change
            # _stock = _stock.groupby(_consumption_saved.index.names).sum().loc[_consumption_saved.index]
            def sort_cum(df):
            ...
            _output = {}
            x = _consumption_saved_efficiency.rename('Consumption saved (TWh)')
            y = cost_efficiency.rename('Cost efficiency (euro/kWh)')
            df_efficiency = concat((x, y), axis=1)
            df_efficiency = sort_cum(df_efficiency)
            _output.update({'efficiency': df_efficiency})
            if _output_statistics:
                _output_statistics.update({'efficiency': pd.Series(_output_statistics['efficiency'], index=[closest(df_efficiency, _output_statistics['efficiency'])])})
            ...
            if npv is not None:
                x = _consumption_saved_npv.rename('Consumption saved (TWh)')
                y = npv.rename('Net present value (NPV) (euro)')
                df_npv = concat((x, y), axis=1)
                df_npv = sort_cum(df_npv)
            ...
            if roi is not None:
                x = _consumption_saved_roi.rename('Consumption saved (TWh)')
                y = roi.rename('Return on Investment (ROI) (year)')
                df_roi = concat((x, y), axis=1)
                df_roi = sort_cum(df_roi)
        ...
        if self.year > self.first_year:
            ...
            if self.year == 2019:
                ...
                for key in dict_rslt.keys():
                    for k in dict_rslt[key].keys():
                        ...
                        # if k in dict_stats[key].keys():
                            # dict_stats[key][k].index = dict_stats[key][k].index / (c_before.sum() / 10 ** 9)

                make_plots(dict_rslt['efficiency'], 'Cost efficiency (euro per kWh saved)',
                           save=os.path.join(self.path_ini, 'abatement_curve.png'.format(k)), ymax=1, ymin=-1,
                           format_y=lambda y, _: '{:.1f}'.format(y), loc='left', left=1.25, colors=colors,
                           format_x=lambda x, _: '{:.0%}'.format(x))
                make_plots(dict_rslt['npv'], 'Net present value (Thousand euro)',
                           save=os.path.join(self.path_ini, 'npv_insulation_curve.png'.format(k)),
                ...
                make_plots(dict_rslt['roi'], 'Return Time on Investment (years)',
                           save=os.path.join(self.path_ini, 'roi_insulation_curve.png'.format(k)),
                           format_y=lambda y, _: '{:.1f}'.format(y), loc='left', left=1.25, colors=colors,
            ...
            output.update({
                              'Consumption standard saving insulation (TWh/year)': consumption_saved_insulation.sum().sum() / 10 ** 9})
            _consumption_saved_actual_insulation = self.apply_calibration(
                consumption_saved_actual_insulation.sum(axis=1))
            output.update(
                {'Consumption saving insulation (TWh/year)': _consumption_saved_actual_insulation.sum() / 10 ** 9})
            _consumption_saved_no_rebound_insulation = self.apply_calibration(
                consumption_saved_no_rebound_insulation.sum(axis=1))
            output.update({
                              'Consumption saving no rebound insulation (TWh/year)': _consumption_saved_no_rebound_insulation.sum() / 10 ** 9})
            ...
            if subsidies is not None:
                ...
                for i in subsidies.index:
                    ...
                    output.update(temp.T / 10 ** 3)
                    output['{} (Billion euro)'.format(i.capitalize().replace('_', ' '))] = subsidies.loc[
                                                                                               i] / 10 ** 9 / step

...
class AgentBuildings(ThermalBuildings):
    ...
    @timing
    def endogenous_retrofit(self, index, prices, subsidies_total, cost_insulation, ms_insulation=None, retrofit_rate_ini=None,
        ...
        def to_market_share(bill_saved, subsidies, investment, utility_zil=utility_zil, scale=1.0):
            ...
            # i = (True, True, 'Social-housing', 'D10', 'Single-family', 'Wood fuel-Standard boiler', 2.51, 0.47, 2.11, 4.23, 'Wood fuel-Performance boiler')
            # self.keep[self.year] = pd.concat((utility_bill_saving.loc[i, :], utility_investment.loc[i, :], utility_subsidies.loc[i, :], constant.loc[i, :]), axis=1)
        ...
        if detailed:
            ...
            p, a = calculate_financial_indicator(insulation_private_optim, consumption, consumption_before, bill_saved,
                                                 cost_insulation, subsidies_total, npv, agent.certificate, stock,
                                                 name='private_optim')
            insulation_max_saving = consumption_saving.idxmax(axis=1)
            payback, abatement = calculate_financial_indicator(insulation_max_saving, consumption, consumption_before,
                                                               bill_saved, cost_insulation, subsidies_total, npv,
    ...
    def flow_retrofit(self, prices, cost_heater, ms_heater, cost_insulation, ms_insulation, ms_extensive,
        ...
        logging.debug('Agents: {}'.format(stock.shape[0]))

...
class AgentBuildings(ThermalBuildings):
    ...
    def heater_replacement(self, stock, prices, cost_heater, lifetime_heater, policies_heater, ms_heater=None,
        ...
        information_rate = 1/3
        flow_premature_replacement = 0
        if premature_replacement is not None:

            def premature_func(x, x0, alpha):
                return 1 / (1 + exp(alpha * (x - x0)))

            def calibration_premature(x, stock, time, mean=3, std=1):
                proba = premature_func(time, x[0], x[1])
                replace = proba * stock.reindex(proba.index)
                mean_calc = (replace * time).sum() / replace.sum()
                std_calc = (((time - mean) ** 2 * replace).sum() / (replace.sum() - 1)) ** (1 / 2)
                return mean_calc - mean, std_calc - std
            ...
            time = time.dropna(axis=1, how='all').squeeze()
            time = time[time < 10]
            fsolve(calibration_premature, (3, 1), args=(stock, time))
            calibration_premature((3, 0.5), stock, time)


            flow_replace = time[(time < premature_replacement)]
    ...
    def endogenous_renovation(self, stock, prices, subsidies_total, cost_insulation,
        ...
        def to_expected_utility(_cost_total, _bill_saved, _subsidies_total, _market_share, _utility_intensive=None,
            ...
            if option == 'log_sum':
                ...
                if _financing_insulation is not None:
                    ...
                    pref_financing = reindex_mi(self.preferences_insulation['cost'], _financing_insulation.index).rename(None)
        ...
        def apply_endogenous_renovation(_bill_saved, _subsidies_total, _cost_total, _cost_financing=None, _stock=None,
            ...
            if _supply:
                ...
                def price_elasticity_demand_bis(price, u=utility):
                    proba = 1 / (1 + exp(-u))
                    elasticity = pref_investment * price / 1000 * (1 - proba)
                    elasticity_global = (elasticity * weight).sum()
                    return elasticity_global
                ...
                root, info_dict, ier, mess = fsolve(cournot_equilibrium, cost_renovation * _supply['markup'],
                                                    args=(self.number_firms, cost_renovation, utility),
                                                    full_output=True)
                print(flow_renovation_before)
                print(cost_renovation)
                print(self.number_firms)
                proba = 1 / (1 + exp(-utility))
                elasticity = pref_investment * cost_renovation * _supply['markup'] / 1000 * (1 - proba)
                elasticity_global = (elasticity * weight).sum()
                ...
                if mu != 0:
                    ...
                    self.logger.info('ERROR No Equilibrium')
                    """
                    for i in range(50):
                        print((1 + i / 10))
        ...
        def calibration_coupled(_stock, _cost_total, _bill_saved, _subsidies_total, _calib_renovation,
            ...
            for k in range(10):
                ...
                if self._expected_utility != 'log_sum' or utility_intensive is None:
                    ...
                    constant, scale = calibration_extensive(utility_intensive, _stock, _calib_renovation, option='log_sum')
                ...
                if self._expected_utility != 'log_sum':
                    expected_utility = to_expected_utility(_cost_total, _bill_saved, _subsidies_total, _market_share,
                                                           _utility_intensive=utility_intensive, _cost_financing=_cost_financing)
                else:
        ...
        def assess_sensitivity(_stock, _cost_total, _bill_saved, _subsidies_total):
            ...
            make_plots(dict_df, 'Average cost (Thousand euro)', save=os.path.join(self.path_calibration, 'sensi_average_cost.png'))
            ...
            make_plots(dict_df, 'Flow renovation (Thousand)', save=os.path.join(self.path_calibration, 'sensi_flow_renovation.png'))
        ...
        if self.constant_insulation_intensive is None and self._threshold is False:
            ...
            if False:
                if self.path_calibration is not None:
                    assess_policies(stock, subsidies_details, cost_total, bill_saved, subsidies_total)
    ...
    def store_information_insulation(self, condition, cost_insulation_raw, tax, cost_insulation, cost_financing,
        ...
        self._renovation_store.update({
            ...
            'cost_households': cost_insulation * self._markup_store,
            'cost_financing_households': cost_financing,
            'vta_households': vta_insulation,
            'subsidies_households': subsidies_total,
            'subsidies_details_households': subsidies_details,
            'cost_component': cost_insulation_raw * self.surface_insulation * (1 + tax) * self._markup_store,
    ...
    def parse_output_run(self, prices, inputs, climate=None, step=1, taxes=None, detailed_output=True,
        ...
        if self.year > self.first_year:
            ...
            output['Markup insulation'] = self._markup_store

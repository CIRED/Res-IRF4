...
`"value"` is the rebate in percent.
...
In the config files add at the same level that the Reference scenario.
1. Policy to assess need to be in the Reference scenario.
2. `quntiles` must be false.
```
"Reference": {
    ...
    "quntiles": false,
...
In the `"assessment"` dictionary:
- Set `activated: "true"`
- Define the name of the policy that need to be assessed
- Set `"AP": "Reference"`

```
"assessment": {
    "activated": true,
    "Policy name": "global_retrofit",
    "AP": "Reference",
    "AP-1": true,
    "ZP": true,
    "ZP+1": true,
...
To assess regulation policies - without direct public cost of the policy `"AP-20XX":  false`.
Otherwise, the model will automatically calculate cost-effectiveness.


## Current policies

The parameterization elements detailed below are summarized in {numref}`public_policies_summary`. 
